...
def get_thumbnail(old_image_id, img, i_corpus, image_id, bb, # python3
    ...
    if 'saiapr' in code_icorpus[i_corpus]:
        img_cropped = img[y:y+h,x:x+w] 
    elif 'mscoco' in code_icorpus[i_corpus]:
        img_cropped = img[int(y):int(y)+int(h),int(x):int(x)+int(w)]
...
def filter_by_fileregionlist(refdf, filelist):
    return pd.merge(refdf, pd.DataFrame(filelist, columns=['image_id','region_id']))
...
def filter_X_by_refdf(X, refdf):
    df_X = pd.DataFrame(X)
    df_refdf = pd.DataFrame(list(set([tuple(e) for e in \
                                      refdf.iloc[:,:3].values.tolist()])))
    # df_merged = pd.merge(df_refdf.iloc[:,:3], df_X, how='left', 
    #                      left_on=[0,1,2],
    #                      right_on=[0,1,2])
    df_merged = pd.merge(df_X, df_refdf.iloc[:,:3], how='right', 
                         left_on=[0,1,2],
                         right_on=[0,1,2])
    X_tmp = df_merged.values
    # if there was a row in refdf not in X, it now contains NaNs
    # drop those first:
    return X_tmp[~np.any(np.isnan(X_tmp), axis=1)]
