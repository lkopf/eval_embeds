...
nltk.download('averaged_perceptron_tagger') # might not need this
...
refcocopdf = pd.DataFrame(refcocop)

refpdf_list = []
for n, this_row in refcocopdf.iterrows():
    this_file = this_row['image_id']
    this_region = this_row['ann_id']
    for this_sentence, this_rexid in zip(this_row['sentences'],
                                         this_row['sent_ids']):
        this_sentence_sent = this_sentence['sent']
        refpdf_list.append((this_file, this_region,
                           this_sentence_sent, this_rexid))
        
refcocopdf_tmp = pd.DataFrame(refpdf_list, 
                             columns='image_id region_id refexp rex_id'.split())

refcocopdf_tmp['i_corpus'] = icorpus_code['mscoco']
refcocopdf_tmp['r_corpus'] = 'refcocop'

refcocopdf_tmp['tagged'] = refcocopdf_tmp['refexp'].apply(postag)
refcocop_fin = refcocopdf_tmp[['i_corpus', 'image_id', 'region_id', 
                             'r_corpus', 'rex_id', 'refexp', 'tagged']]
...
print refcocop_fin.head(1)

print 'refdf: %d\nrefcoco: %d\ngex: %d\nrefcocop: %d' % (len(refdf), 
                                           len(refcoco_fin), len(gexdf), len(refcocop_fin))
